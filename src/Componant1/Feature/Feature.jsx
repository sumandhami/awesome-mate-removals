...
const icons = [
  <FiTruck size={34} className="text-SecondaryColor-0" />,
  <FiClock size={34} className="text-SecondaryColor-0" />,
  <FiShield size={34} className="text-SecondaryColor-0" />,
];

const fallbackFeatures = [
  { id: 1, img: icons[0], title: "★ Licensed Perth Business", desc: "Registered and operating locally across Perth suburbs." },
  { id: 2, img: icons[1], title: "4+ Years in Business", desc: "Experienced local Perth moving crew." },
  { id: 3, img: icons[2], title: "Fully Insured", desc: "Transit and handling coverage for home and commercial moves." },
];

const Feature = ({ features = null }) => {
  const items = features && features.length > 0
    ? features.map((f, i) => ({ id: i, img: icons[i] ?? icons[0], title: f.title, desc: f.description }))
    : fallbackFeatures;
