    ...
    return (
      <div className="bg-[#f3f4f8] group transition-all duration-500 hover:bg-white hover:shadow-cases rounded-md">
        ...
        <div className="px-5 sm:px-7 pb-7">
          <Link to={blogUrl} className="font-Inter text-left font-semibold text-xl sm:text-[22px] text-HeadingColor-0 transition-all duration-500 group-hover:text-SecondaryColor-0 mt-7 mb-3 block">
              {blogTitle}
          </Link>
          <p className="font-Poppins font-light text-TextColor-0">{blogDesc}</p>
          <Link to={blogUrl} className="flex gap-2 items-center font-Inter font-medium text-HeadingColor-0 transition-all duration-500 border-b border-transparent group-hover:text-SecondaryColor-0 group-hover:border-SecondaryColor-0 mt-5">
              {btnContent}
              {btnIcon}
          </Link>
        </div>
